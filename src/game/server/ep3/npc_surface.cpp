...
void CLennardJonesForce::AddForces(IPhysicsObject** pObject, int nObjects, float flRadius, float flStrength, Vector* pForces)
{
	...
	//[EP3T] should this be a class variable still?
	PhysTiler * m_pPhysTiler = PhysTilerFactory::factory->getTiler();

	m_pPhysTiler->setCacheParams(m_fInteractionRadius,0.0f);//[EP3T]
...
void CNPC_BlobArmTest::RunAI(void)
{
	...
	PhysTiler* m_pPhysTiler = PhysTilerFactory::factory->getTiler();
	m_pPhysTiler->setCacheParams(sv_surface_nearby.GetFloat(), 0.0f);//[EP3T]
	...
	while(p1 != NULL)
	{
		...
		//PhysParticleAndDist* node = pCache->get(p1);
...
void CNPC_BlobDemoMonster::RepulseNeighbors()
{
	...
	while (p1 != NULL)
	{
		...
		//numNeighbours = 0; //[EP3T] uncomment this to disable the below code
