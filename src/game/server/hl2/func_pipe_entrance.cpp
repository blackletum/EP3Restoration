...
{
    //Oh its probably removing itself from its connected
    // 
    ...
    for (int i = 0; i < field16_0x550.Count(); i++)
    {
        CUtlVector<storer> & connectedList = field16_0x550[i].pPipe->field16_0x550;
        for (int j = 0; j < connectedList.Count(); j++)
        {
            if (connectedList[j].pPipe == this)
            {
                connectedList.Remove(j);
                break;
            }
        }
    }
