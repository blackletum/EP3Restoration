...
ConVar sv_special_suit_abilities_toggle("sv_special_suit_abilities_toggle", "0", 0, "Set this to have the ALT1 key toggle the suit ability on and off.");
ConVar sv_special_suit_abilities("sv_special_suit_abilities", "1"); //Appears to be unused. Maybe weapons themselves were meant to check this in IsSpecialSuitAbility?
...
//SendPropDataTable("hl2localdata", 0, &REFERENCE_SEND_TABLE(DT_HL2LocalData), SendProxy_SendLocalDataTable),
// Data that gets sent to all other players
//SendPropDataTable("hl2nonlocaldata", 0, &REFERENCE_SEND_TABLE(DT_HL2NonLocal), SendProxy_SendNonLocalDataTable),
END_SEND_TABLE()
...
void CHL2_Player::PreThink(void)
{
	...
	if (m_afPhysicsFlags & PFLAG_IMMOBILIZED)
	{
		//[EP3T] It seems that Robot Repair 
		Warning("Player no longer immobilized?\n");
		Assert(0);
		m_afPhysicsFlags &= ~PFLAG_IMMOBILIZED;
	}
	
	if (m_afPhysicsFlags & PFLAG_ONBARNACLE)//RR removed this. why?
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CHL2_Player::FlashlightTurnOn(bool playSound)
{
...
void CHL2_Player::SetActiveSpecialSuitAbility(CBaseCombatWeapon* pAbility)
{
	if(pAbility)
		m_hActiveSpecialSuitAbility = pAbility;
}

CBaseCombatWeapon* CHL2_Player::GetActiveSpecialSuitAbility() const
{
	return m_hActiveSpecialSuitAbility;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHL2_Player::FlashlightTurnOff(bool playSound)
{
	if (Flashlight_UseLegacyVersion())
	{
...
void CHL2_Player::SpecialSuitAbilityPostFrame()
{
	CBaseCombatWeapon * plVar5 = GetActiveSpecialSuitAbility();
	if (plVar5 == NULL) 
	{
		return;
	}
	...
	bool bVar3 = false;
	bool bVar4 = false;
	m_afButtonReleased = m_afButtonReleased & ~IN_ATTACK;
	m_nButtons = m_nButtons & ~IN_ATTACK;
	if (m_nButtons & IN_ALT1) 
	{
		if (sv_special_suit_abilities_toggle.GetBool() == 0) 
		{
			bVar3 = true;
		}
		else if (!m_bAlt1ToggledNotReady) 
		{
			bool bVar6 = m_bAlt1ToggledOn == false;
			m_bAlt1ToggledNotReady = true;
			m_bAlt1ToggledOn = bVar6;
	...
	{
		if (sv_special_suit_abilities_toggle.GetBool()) 
		{
			m_bAlt1ToggledNotReady = false;
		}
		else 
		{
			bVar4 = true;
	...
	if (sv_special_suit_abilities_toggle.GetBool() && m_bAlt1ToggledOn) 
	{
		bVar3 = true;
	}
	else 
	{
		if (!bVar3) goto LAB_10abc2810;
	}
	m_nButtons = m_nButtons | 1;
LAB_10abc2810:
	if (bVar4) 
	{
		m_afButtonReleased = m_afButtonReleased | 1;
	}
	if (bVar3 || bVar4) 
	{
		plVar5->ItemPostFrame();
	}
